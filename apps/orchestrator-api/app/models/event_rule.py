...
class BackupEventRule(Base):
    ...
    db_config: Mapped["BackupEventRuleDbConfig | None"] = relationship(back_populates="rule", uselist=False, cascade="all, delete-orphan")
    s3_config: Mapped["BackupEventRuleS3Config | None"] = relationship(back_populates="rule", uselist=False, cascade="all, delete-orphan")


class BackupEventRuleDbConfig(Base):
    __tablename__ = "backup_event_rule_db_configs"
    ...
    rule: Mapped[BackupEventRule] = relationship(back_populates="db_config")


class BackupEventRuleS3Config(Base):
    __tablename__ = "backup_event_rule_s3_configs"
    ...
    rule: Mapped[BackupEventRule] = relationship(back_populates="s3_config")
